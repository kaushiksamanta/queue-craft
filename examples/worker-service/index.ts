...
const recordInvalidPhoneNumber = (phoneNumber: string): Promise<void> => {
...
const processDeadLetterMessage = async (payload: any, metadata: MessageMetadata): Promise<void> => {
  ...
  try {
    ...
    logger.error(`Error in dead letter processing:`, {
      error: error instanceof Error ? error.message : 'Unknown error',
    })
  }
}

const runWorkerDemo = async () => {
  ...
  try {
      ...
      handlers: {
        'notification.email': async (payload, metadata) => {
          ...
          try {
            ...
            logger.error(`Error in email processing pipeline:`, {
              error: error instanceof Error ? error.message : 'Unknown error',
            })
            throw error
          }
    ...
    await Promise.all([basicWorker.start(), errorHandlingWorker.start(), deadLetterWorker.start()])
